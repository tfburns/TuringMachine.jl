...
state, tape_left, tape_right = TuringMachine.set_up_perceptron(init_state, input)

# run simulation step-by-step until we reach a halting state of 'accept' or 'reject'
while true
    try
        global state, tape_left, tape_right = TuringMachine.simulate(state, program, tape_left, tape_right)
        println(state)
        state ==("qAccept") && break
        state ==("qReject") && break
    catch err
